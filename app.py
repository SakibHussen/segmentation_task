...
#     """
#     Converts model prediction to a segmentation mask image.
#     """
#     mask = np.argmax(mask[0], axis=-1).astype(np.uint8)  # (96, 128)

#     if colorize:
#         # Apply matplotlib colormap (normalized to [0,1])
#         cmap = cm.get_cmap("nipy_spectral")
#         color_mask = cmap(mask / float(N_CLASSES - 1))[:, :, :3]  # drop alpha
...
#     st.write("Upload an image and its true label mask (optional), then click Predict.")

#     uploaded_file = st.file_uploader("Choose an image...", type=["jpg", "jpeg", "png"], key="input_image")
#     label_file = st.file_uploader("Upload the true label mask (optional)...", type=["jpg", "jpeg", "png"], key="label_image")
...
#         label_image = label_image.resize((IMG_WIDTH, IMG_HEIGHT))
...
#         if image is None:
#             st.warning("Please upload an input image before predicting.")
#         else:
